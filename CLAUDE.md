...
Este repositório é o **Dr. Juliano Fratezi Conteúdo Studio**: edição e agendamento de conteúdo para as redes da **Dr. Juliano Fratezi**. Estúdio da agência com infraestrutura compartilhada; o posicionamento é o desta marca.
...
- Estúdio: este repo (symlink `~/drjulianofratezi-conteudo` aponta para cá). Projetos em `projects/<nome>/`.
...
- Metricool: conta da agência (userId 5085980). Marca no painel: **drjulianofratezi** (blog_id **6741542**), timezone America/Sao_Paulo. Redes conectadas: **nenhuma ainda (PENDENTE conectar Instagram e demais canais no painel do Metricool)**. Melhor horário de publicação: medir com getBestTimeToPostByNetwork após conectar.
...
- Kairogen: conta da agência, conector OK. Créditos em 2026-08-18: **0 (PENDENTE recarga antes de usar para b-roll)**.
- ElevenLabs: chave `sk_` (51 chars) na env var `ELEVENLABS_API_KEY` do environment; o setup grava em `.env` na raiz do video-use. Escopos necessários: TTS, STT Scribe, sound-generation, voices_read (todos validados em 2026-08-18). Plano: **free, 10.000 créditos/mês** (261 usados em 2026-08-18; workspace "Zavi"). Voz da marca para narração: **PENDENTE (voice_id, modelo e settings)**.
...
- Cloud, environment desta conta (validado 2026-08-18): apt-get é bloqueado pelo proxy (403 em HTTP); o setup.sh instala ffmpeg estático do BtbN via github releases (github.com e *.githubusercontent.com de assets passam; `raw.githubusercontent.com` NÃO passa localmente). Consequência: `npx hyperframes skills update` falha no manifest remoto; o setup.sh registra as skills por symlink do clone local. O bloqueio local do raw.githubusercontent.com não afeta a rota de mídia pública para o Metricool (o fetch é feito pelos servidores deles).
